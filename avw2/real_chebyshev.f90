...
program main 
  ...
  open (unit=10, file='errors.txt', status='unknown')
	...
	allocate(wpoly(N+1),wroots(N),wcoeffs(N,3))
  	...
  	wCoeffs = complex(0d0,0d0)
  	wCoeffs(:,1) = complex(.5d0,0d0)
  	wCoeffs(:,3) = complex(.5d0,0d0)
  	wCoeffs(N,1) = complex(1d0,0d0)
  	wCoeffs(N,3) = complex(0d0,0d0) 
	...
	call DAVW2(1,N,3,POLY,COEFFS,RROOTS,IROOTS,ITERATIONS,FLAG)
	
	do kk=1,N
		wroots(kk) = complex(rroots(kk),iroots(kk))
	end do

	call RESCHECK(1,N,3,1,wPOLY(2:N+1)/wpoly(1),wCOEFFS,wROOTS,ALLROOTS,RES)	
