...
program main 
  ...
  open (unit=10, file='errors.txt', status='unknown')
	...
	allocate(poly(N+1),roots(N),coeffs(N,3),allroots(N,2),iterations(N),res(N,6))
  	...
  	Coeffs(:,1) = complex(.5d0,0d0)
  	Coeffs(:,3) = complex(.5d0,0d0)
  	Coeffs(N,1) = complex(1d0,0d0)
  	Coeffs(N,3) = complex(0d0,0d0) 
	...
	call ZAVW2(1,N,3,POLY,COEFFS,ROOTS,ITERATIONS,FLAG)
	call RESCHECK(1,N,3,1,POLY(2:N+1)/poly(1),COEFFS,ROOTS,ALLROOTS,RES)	
